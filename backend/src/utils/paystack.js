...
class PaystackClient {
  ...
  async initializeTransaction({ email, amount, callbackUrl, reference, dataCategory = 'real' }) {
    console.log(`[Paystack] Initialize transaction for ${email} - ${amount} ZAR [Category: ${dataCategory}] [Ref: ${reference}]`);
    ...
    if (dataCategory !== 'real' || this.secretKey === 'stub_key') {
      return {
        ...
        data: {
          ...
          reference: reference || `stub-ref-${Date.now()}`
        }
      };
    }
