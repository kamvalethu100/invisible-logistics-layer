...
export async function paymentRoutes(fastify, options) {
  ...
  fastify.post('/payout', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
        ...
        const user = await db.get('SELECT country_code, currency_code, data_category, phone, verification_status FROM users WHERE id = ?', [request.user.id]);
        ...
        await db.run(
            `INSERT INTO payments (id, user_id, amount, currency, status, provider, type, data_category, metadata) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [payoutId, request.user.id, -data.amount, user.currency_code, 'PENDING', provider, 'PAYOUT', user.data_category, JSON.stringify({ delivery_id: data.delivery_id, external_response: externalResponse })]
        );

        return { 
            status: 'success', 
            payout_id: payoutId, 
            message: `Payout initiated via ${provider}`,
            external_response: externalResponse
        };
    } catch (error) {
        return reply.status(400).send({ error: error.message });
  ...
  fastify.post('/callback/paystack', async (request, reply) => {
      ...
      if (event === 'charge.success') {
          ...
          if (payment.status !== 'PENDING') {
              fastify.log.info(`[Paystack Callback] Payment ${reference} already processed (Status: ${payment.status})`);
              return { status: 'success', already_processed: true };
          }
