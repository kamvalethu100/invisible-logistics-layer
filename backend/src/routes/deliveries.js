...
export async function deliveryRoutes(fastify, options) {
    ...
    try {
      ...
      await db.run(
        ...
        [
          ...
          data.dropoff_address, data.dropoff_lat, data.dropoff_lng, data.package_size, data.urgency, price, data_category, country_code, currency_code, region, data.insurance_opt_in ? 1 : 0, insurance_premium, 'pending'
        ]
      );

      const delivery = await db.get('SELECT * FROM deliveries WHERE id = ?', [id]);
      
      // LOGISTIQS LIVE PHASE: Matching is NOT triggered on creation.
      // Users must first "Proceed to Pay" to enter the settlement gate.
  ...
  // Initiate payment (Settlement Gate)
  fastify.patch('/:id/initiate-payment', async (request, reply) => {
    ...
    // Notify matching engine via persistent loop (matching.js will pick it up)
    console.log(`[Settlement] Payment initiated for delivery ${id}. Entering matching queue.`);

    return { success: true, message: 'Payment initiated. Banking details authorized.' };
  });
